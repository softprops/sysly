...
fn main() {
  ...
  let stream_result = stream.write_line(
    "66 <174>1 2012-07-22T00:06:26-00:00 somehost Rust console 2 Hi from Rust");
  match stream_result {
      Err(e) => panic!("err {}", e),
      Ok(_) => println!("stream send result ok")
  };

  let udp_result = udp.send_to(
    &"66 <174>1 2012-07-22T00:06:26-00:00 somehost Rust console 2 Hi from Rust".as_bytes(),
   addr);
  match udp_result {
      Err(e) => panic!("err {}", e),
